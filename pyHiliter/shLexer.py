...
from pygments.token import Punctuation, STANDARD_TYPES,\
...
class BashLexer(RegexLexer):
    ...
    tokens = {
        ...
        'interp': [
            (r'\$\(\(', Punctuation, 'math'),
            (r'\$\(', Punctuation, 'paren'),
            (r'(\$\{)(#?)', bygroups(String.Interpol, Operator), 'curly'),
            (r'\$[a-zA-Z_]\w*', Name.Variable),  # user variable
            (r'\$(?:\d+|[#$?!_*@-])', Name.Variable),      # builtin
            (r'\$', Text),
        ],
        'basic': [
            (r'\b(if|fi|else|while|do|done|for|in|then|return|case|local|'
             r'select|continue|until|esac|elif)(\s*)\b',
             bygroups(Keyword, Text)),
            (r'\b(alias|apt-get|awk|bg|bind|break|brew|builtin|caller|cat|cd|'
             r'command|compgen|complete|conda|cp|cut|declare|diff|dirs|'
             r'disown|docker|du|echo|enable|eval|exec|exit|export|expr|fc|fg|'
             r'getopts|git|grep|hash|head|help|history|jobs|kill|let|ln|'
             r'logout|ls|lsof|mkdir|mv|perl|pip3?|popd|printf|pushd|pwd|'
             r'python3?|read|readonly|rm|rmdir|sed|set|shift|shopt|sleep|'
             r'sort|source|su|sudo|suspend|tail|tee|test|time|times|touch|'
             r'trap|type|typeset|ulimit|umask|unalias|uniq|unset|wait|which|'
             r'xargs)(?=[\s)`])', Name.Builtin),
            (r'\b(function)(\s+)([-_\w\d]+)', 
              bygroups(Keyword.Declaration, Text, Name.Function)),
            (r'\b([-_\w\d]+)(\(\))', bygroups(Name.Function, Punctuation)),
            (r'true|false(?=[\s)`])', Keyword.Constant),
            (r'--?[a-zA-Z\d_-]+(?=\s*)', Keyword.Argument),
            (r'\A#!.+\n', Comment.Hashbang),
            (r'#.*\n', Comment.Single),
            (r'\\[\w\W]', String.Escape),
            (r'(\b\w+)(=)', bygroups(Name.Variable, Operator), 'string-valueAssign'),
            (r'=', Operator),
            (r'[\[\]{}()]', Punctuation),
        ...
        'string-valueAssign': [
            ...
            (r'(\$(?!\{).*?)(;)', bygroups(Name.Variable, Operator), '#pop'),
            (r'(\$(?!\{).*?)(\s+)', bygroups(Name.Variable, Text.Yo), '#pop'),
            (r'[\w\d@#%^-_+={}+:,./?*]', String),
            default('#pop')
        ],
        'data': [
        ...
        'curly': [
            ...
            (r':-', Keyword),
            (r'\d+', Number.Integer),
            (r'(\[)(@)(\])', bygroups(Punctuation, Operator, Punctuation)),
            (r'\w+', Name.Variable),
            (r'[^}:"\'`$\\]+', Punctuation),
            (r':', Operator),
            include('root'),
...
class BatchLexer(RegexLexer):
    ...
    def _make_redirect_state(compound,
        ...
        return [
            (r'((?:(?<=[%s%s])\d)?)(>>?&|<&)([%s%s]*)(\d)' %
             (_nl, _ws, _nl, _ws),
             bygroups(Number.Integer, Operator, Text, Number.Integer)),
            (r'((?:(?<=[%s%s])(?<!\^[%s])\d)?)(>>?|<)(%s?%s)' %
             (_nl, _ws, _nl, _space, stoken_compound if compound else _stoken),
             bygroups(Number.Integer, Operator, using(this, state='text')))
        ]
